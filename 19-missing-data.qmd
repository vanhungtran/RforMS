...
> **What this produced:** `imp_knn` is a matrix with the same dimensions as `mat_raw` (3006 proteins × 12 samples), but with all missing values filled by averaging the k nearest neighbours. Each missing cell is replaced by the mean of the 10 most similar proteins' observed values. This preserves the correlation structure between proteins — if two proteins co-vary, their imputed values will reflect that relationship.
...
> **What this produced:** Each missing value is replaced by a random draw from a Gaussian distribution shifted 1.8 standard deviations below the observed mean for that column, with width 0.3 SD. This places imputed values in the low-intensity tail — mimicking the biological truth that undetected proteins are genuinely low-abundance. The random component (SD = 0.3) prevents all imputed values from landing at the same spot.
...
> **What this produced:** Each protein is imputed differently depending on its missingness rate. Proteins with fewer than 50% missing values are imputed by k-NN (preserving correlations). Proteins with 50% or more missing values are imputed by minimum value (treating absence as biological). This is the most realistic strategy for heterogeneous MS data where both MAR and MNAR mechanisms coexist.
...
```{r}
#| fig-cap: "Density of log2 intensities for one sample under each imputation strategy. The raw data (with NAs) shows the observed distribution. Min-value imputation adds a spike at the minimum. k-NN preserves the distribution shape. Left-censored draws fill the low-intensity tail. Mixed combines both approaches."
