...
The `scp` package provides `scp_qc()` for computing per-cell quality metrics:
...
```{r}
#| eval: true
# Number of PSMs identified per channel
qc$nFeatures |>
    as.data.frame() |>
    head()
```
...
```{r}
#| eval: true
# Aggregate PSMs to peptides (group by peptide Sequence)
...
# Aggregate peptides to proteins (group by protein accession)
leduc <- aggregateFeatures(leduc,
    i = "peptides",
    fcol = "Proteins",
    name = "proteins",
...
cat("Protein assay dimensions:", dim(leduc[["proteins"]]), "\n")
```
...
leduc <- normalizeScp(leduc,
    i = "proteins",
    name = "proteins_norm",
    method = "div.median")

cat("Normalized protein assay dimensions:", dim(leduc[["proteins_norm"]]), "\n")
```
...
before <- assay(leduc[["proteins"]])
after <- assay(leduc[["proteins_norm"]])
...
prots_log <- log2(assay(leduc, "proteins_norm") + 1)
...
pca_data$SampleType <- colData(leduc)$SampleType
...
if (requireNamespace("variancePartition", quietly = TRUE)) {
    ...
    varPart <- fitExtractVarPartModel(prots_log, form, colData(leduc))
...
for (cond in unique(colData(leduc)$SampleType)) {
    idx <- colData(leduc)$SampleType == cond
...
library(limma)
design <- model.matrix(~ 0 + colData(leduc)$SampleType)
colnames(design) <- levels(factor(colData(leduc)$SampleType))
fit <- lmFit(prots_imp, design)
cont <- makeContrasts(
    naive_vs_primed = naive - primed,
    levels = design
)
...
tt$sig <- ifelse(tt$adj.P.Val < 0.05 & abs(tt$logFC) > 1,
    ifelse(tt$logFC > 0, "Up in naive", "Down in naive"),
    "Not significant"
)
...
ggplot(tt, aes(x = logFC, y = -log10(adj.P.Val), color = sig)) +
    ...
    scale_color_manual(values = c("Down in naive" = "#2166AC",
                                   "Not significant" = "grey60",
                                   "Up in naive" = "#B2182B")) +
    labs(x = "log2 fold change (naive / primed)",
         y = "-log10(adjusted p-value)",
         title = "Naive vs. primed pluripotent states") +
    theme_minimal() +
...
```{r}
#| eval: true
cat("Top differentially abundant proteins (naive vs. primed):\n")
tt |>
